...
3. 打开 **设置**，填写服务地址、模型名称及密钥，测试连接并保存。已有 Claude 登录的用户可显式选择“使用本机 Claude 登录”。
4. 上传 TXT、DOCX 或文字型 PDF，点击 **开始审查**。完成后下载 Markdown/JSON 报告，在 **人工复核** 中逐项确认并提交。
5. 使用完毕，双击 **停止应用.vbs**。关闭浏览器标签不会停止后台审查。

未配置模型时，可直接查看标注为合成案例的示例报告；该操作不调用模型、不产生正式复核记录。正式审查和连接测试需要联网，使用所配置模型账户的额度。材料在本机解析，提取文本会发送到所配置的模型服务。
...
界面设置优先于源码目录 `.env` 和环境变量；尚未保存界面设置时才读取后两者。设置中的高级选项支持北大法宝 MCP，只启用同时具备 URL 和令牌的连接。未配置 MCP 时使用本地语料，不继承开发者个人项目设置。
...
Git 新增提交使用 `Anonymous Contributor <anonymous@example.invalid>` 作为作者和提交者；不改写既有共享历史和第三方署名。
